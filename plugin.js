...
(function pathBRuntime(g) {
    ...
    async function readDoc(data, pluginId) {
        ...
        const r = records.find(
            (x) => (x.text?.("plugin_id") || "").trim() === pluginId,
        );
        if (!r) return null;
        let raw = "";
        try {
    ...
    function showFirstRunDialog(ui, label, preferred, onPick) {
        const id =
            "thymerext-pathb-first-" + Math.random().toString(36).slice(2);
        ...
        const mk = (t, sub, prim) => {
            ...
            b.style.cssText =
                ...
                (prim ? "var(--bg-selected,var(--bg-hover,rgba(0,0,0,0.08)))" : "transparent") +
                ";color:inherit;";
        ...
        const bSyn = mk(
            "Sync via Plugin Settings",
            "Workspace collection “" + COL_NAME + "”.",
            preferred === "synced",
        );
        const fin = (m) => {
            try {
    ...
    g.ThymerExtPathB = {
        ...
        scheduleFlush(plugin, mirrorKeys) {
            ...
            plugin._pathBFlushTimer = setTimeout(() => {
                ...
                if (!pid || !data) return;
                g.ThymerExtPathB.flushNow(data, pid, keys).catch((e) =>
                    console.error("[ThymerExtPathB] flush", e),
                );
            }, 500);
        },
        ...
        async openStorageDialog(opts) {
            ...
            ui.addToaster?.({
                title: label,
                message:
                    "Storage: " + (pick === "synced" ? "synced" : "local only"),
                dismissible: true,
...
class Plugin extends AppPlugin {
    onLoad() {
        (async () => {
            ...
            await (globalThis.ThymerExtPathB?.init?.({
                ...
                (console.warn(
                    "[Backreferences] ThymerExtPathB runtime missing (redeploy full plugin .js from repo).",
                ),
                Promise.resolve()));
            ...
            this._storageKeyPropertyRefsCollapsed =
                "thymer_backreferences_property_refs_collapsed_v1";
            this._propertyRefsCollapsed = this.loadBoolSetting(
                this._storageKeyPropertyRefsCollapsed,
                false,
            );

            this._storageKeyLinkedRefsCollapsed =
                "thymer_backreferences_linked_refs_collapsed_v1";
            this._linkedRefsCollapsed = this.loadBoolSetting(
                this._storageKeyLinkedRefsCollapsed,
                false,
            );

            this._storageKeyUnlinkedCollapsed =
                "thymer_backreferences_unlinked_collapsed_v1";
            this._unlinkedCollapsed = this.loadBoolSetting(
                this._storageKeyUnlinkedCollapsed,
                false,
            );
            ...
            this._storageKeySortByRecord =
                "thymer_backreferences_sort_by_record_v1";
            this._legacyStorageKeySortByRecord =
                "thymer_backlinks_sort_by_record_v1";
            ...
            this._storageKeyIgnoreCleanupDone =
                "thymer_backreferences_ignore_cleanup_v1";
            ...
            this._eventHandlerIds.push(
                this.events.on("panel.navigated", (ev) =>
                    this.handlePanelChanged(ev.panel, "panel.navigated"),
                ),
            );
            this._eventHandlerIds.push(
                this.events.on("panel.focused", (ev) =>
                    this.handlePanelChanged(ev.panel, "panel.focused"),
                ),
            );
            this._eventHandlerIds.push(
                this.events.on("panel.closed", (ev) =>
                    this.handlePanelClosed(ev.panel),
                ),
            );
            this._eventHandlerIds.push(
    ...
    handlePanelChanged(panel, reason) {
        ...
        if (
            recordChanged ||
            !this.isValidSortBy(state.sortBy) ||
            !this.isValidSortDir(state.sortDir)
        ) {
        ...
        this.scheduleRefreshForPanel(panel, {
            force: recordChanged,
            reason:
                reason || (recordChanged ? "record-changed" : "record-same"),
        });
        this.syncStatusItem();
    }
    ...
    shouldSuppressInPanel(panel, panelEl) {
        ...
        const navType =
            nav && typeof nav.type === "string" ? nav.type.trim() : "";
    ...
    mountFooter(panel, state) {
        ...
        if (needsRebuild) {
            ...
            state.chipsRowEl =
                state.rootEl.querySelector(".tlr-chips-row") || null;
    ...
    findMountContainerDetails(panelEl) {
        ...
        for (const selector of checks) {
            if (panelEl?.matches?.(selector))
                return { element: panelEl, selector };
    ...
    buildFooterRoot(state) {
        ...
        toggleBtn.className =
            "tlr-btn tlr-toggle button-none button-small button-minimal-hover";
        ...
        searchToggle.className =
            "tlr-btn tlr-search-toggle button-none button-small button-minimal-hover";
        searchToggle.type = "button";
        searchToggle.dataset.action = "toggle-search";
        searchToggle.title = "Filter references";
        searchToggle.setAttribute(
            "aria-expanded",
            state.searchOpen === true ? "true" : "false",
        );
        try {
        ...
        input.addEventListener("keydown", (e) => {
            ...
            if (e.key === "Escape") {
                e.preventDefault();
                const hasAny =
                    state.searchTyped || state.searchPhrases.length > 0;
                if (hasAny) {
                    state.searchTyped = "";
                    state.searchPhrases = [];
                    input.value = "";
        ...
        clearBtn.className =
            "tlr-search-clear button-none button-small button-minimal-hover";
        ...
        sortToggle.className =
            "tlr-btn tlr-sort-toggle button-none button-small button-minimal-hover";
        ...
        sortToggle.setAttribute(
            "aria-expanded",
            state.sortMenuOpen === true ? "true" : "false",
        );
        ...
        sortMenu.className =
            "tlr-sort-menu cmdpal--inline dropdown active focused-component";
    ...
    handleFooterClick(e) {
        ...
        if (action === "toggle") {
            ...
            for (const s of this._panelStates.values()) {
                ...
                const btn =
                    s.rootEl.querySelector?.('[data-action="toggle"]') || null;
        ...
        if (action === "toggle-prop-group") {
            ...
            const isCollapsed = groupEl
                ? groupEl.classList.contains("tlr-prop-collapsed")
                : this.isPropGroupCollapsed(propName);
        ...
        if (action === "set-sort-by") {
            ...
            this.applySortPreferenceForRecord(
                state.recordGuid,
                nextSortBy,
                state.sortDir,
            );
        ...
        if (action === "remove-chip") {
            ...
            state.searchPhrases = (state.searchPhrases || []).filter(
                (p) => p !== phrase,
            );
            this.rebuildChips(state);
            this.renderFromCache(state);
        ...
        if (action === "toggle-property-refs") {
            ...
            this.saveBoolSetting(
                this._storageKeyPropertyRefsCollapsed,
                this._propertyRefsCollapsed,
            );
            for (const s of this._panelStates.values()) {
                const el =
                    s.rootEl?.querySelector?.('[data-section="property"]') ||
                    null;
                if (el)
                    el.classList.toggle(
                        "tlr-section-collapsed",
                        this._propertyRefsCollapsed,
                    );
            }
            return;
        ...
        if (action === "toggle-linked-refs") {
            ...
            this.saveBoolSetting(
                this._storageKeyLinkedRefsCollapsed,
                this._linkedRefsCollapsed,
            );
            for (const s of this._panelStates.values()) {
                const el =
                    s.rootEl?.querySelector?.('[data-section="linked"]') ||
                    null;
                if (el)
                    el.classList.toggle(
                        "tlr-section-collapsed",
        ...
        if (action === "toggle-unlinked-refs") {
            ...
            this.saveBoolSetting(
                this._storageKeyUnlinkedCollapsed,
                this._unlinkedCollapsed,
            );
            for (const s of this._panelStates.values()) {
                const el =
                    s.rootEl?.querySelector?.('[data-section="unlinked"]') ||
                    null;
                if (el)
                    el.classList.toggle(
                        "tlr-section-collapsed",
        ...
        if (action === "expand-record") {
            ...
            const groupEl = actionEl.closest?.(".tlr-group") || null;
            if (groupEl)
                this.toggleRecordExpansion(state, guid, groupEl).catch(
                    () => {},
                );
            return;
        }
        ...
        if (action === "toggle-preview-node") {
            ...
            const groupEl = actionEl.closest?.(".tlr-group") || null;
            const previewEl =
                groupEl?.querySelector(".tlr-record-preview") || null;
            if (previewEl)
    ...
    setSearchOpen(state, open) {
        ...
        state.rootEl.classList.toggle(
            "tlr-search-open",
            state.searchOpen === true,
        );
        state.searchToggleEl?.setAttribute?.(
            "aria-expanded",
            state.searchOpen === true ? "true" : "false",
        );
    ...
    applySortPreferenceForRecord(recordGuid, sortBy, sortDir) {
        ...
        const nextSortDir =
            this.normalizeSortDir(sortDir) || this._defaultSortDir;
    ...
    renderSortMenu(state) {
        ...
        ascBtn.type = "button";
        ascBtn.className =
            "tlr-sort-dir-btn button-normal button-normal-hover button-small";
    ...
    saveCollapsedSetting(collapsed) {
        try {
            localStorage.setItem(
                this._storageKeyCollapsed,
                collapsed ? "1" : "0",
            );
        } catch (e) {
            // ignore
        }
        globalThis.ThymerExtPathB?.scheduleFlush?.(this, () => [
            "thymer_backreferences_collapsed_v2",
            "thymer_backreferences_prop_group_collapsed_v2",
            "thymer_backreferences_record_group_collapsed_v1",
            "thymer_backreferences_property_refs_collapsed_v1",
        ...
        const parse = (v) => {
            ...
            try {
                ...
                for (const [recordGuid, pref] of Object.entries(parsed)) {
                    const guid =
                        typeof recordGuid === "string" ? recordGuid.trim() : "";
                    if (!guid) continue;
    ...
    saveSortByRecordSetting() {
        try {
            localStorage.setItem(
                this._storageKeySortByRecord,
                JSON.stringify(this._sortByRecord || {}),
            );
        } catch (e) {
            // ignore
        }
    ...
    setSortPreferenceForRecord(recordGuid, sortBy, sortDir) {
        ...
        if (
            nextSortBy === this._defaultSortBy &&
            nextSortDir === this._defaultSortDir
        ) {
    ...
    hasCompletedIgnoreCleanupMigration() {
        try {
            return (
                localStorage.getItem(this._storageKeyIgnoreCleanupDone) === "1"
            );
        } catch (e) {
            // ignore
        }
    ...
    buildResultsSnapshot(propertyGroups, linkedGroups) {
        ...
        for (const g of propertyGroups || []) {
            ...
            for (const record of g?.records || []) {
                ...
                const key = this.getPropertySnapshotKey(
                    propertyName,
                    recordGuid,
                );
                itemsByKey.set(key, {
    ...
    diffCurrentSnapshotKeys(prevSnapshot, nextSnapshot) {
        ...
        const prevItems =
            prevSnapshot?.itemsByKey instanceof Map
                ? prevSnapshot.itemsByKey
                : new Map();
        const nextItems =
            nextSnapshot?.itemsByKey instanceof Map
                ? nextSnapshot.itemsByKey
                : new Map();

        for (const [key, nextItem] of nextItems.entries()) {
            const prevItem = prevItems.get(key) || null;
            if (!prevItem || prevItem.signature !== nextItem.signature)
                changed.add(key);
        }
    ...
    markStatePendingRemote(state, ev) {
        ...
        if (!(state.pendingRemoteUsers instanceof Set))
            state.pendingRemoteUsers = new Set();

        const user =
            typeof ev.getSourceUser === "function" ? ev.getSourceUser() : null;
    ...
    applyLiveSnapshot(state, snapshot) {
        ...
        const nextRemoteBadges =
            state.liveRemoteBadgesByKey instanceof Map
                ? new Map(state.liveRemoteBadgesByKey)
                : new Map();

        for (const key of Array.from(nextRemoteBadges.keys())) {
            if (!currentSnapshot.itemsByKey.has(key))
                nextRemoteBadges.delete(key);
        }

        if (state.pendingRemoteSync === true) {
            const tooltip = this.getRemoteBadgeTooltip(
                state.pendingRemoteUsers,
            );
            for (const key of this.diffCurrentSnapshotKeys(
                previous,
                currentSnapshot,
            )) {
    ...
    getLiveBadgesForKey(state, itemKey) {
        ...
        if (
            state.liveNewKeys instanceof Set &&
            state.liveNewKeys.has(itemKey)
        ) {
            badges.push({
                label: "New",
                className: "is-new",
                tooltip: "Added since this page was opened",
            });
        }

        if (
            state.liveRemoteBadgesByKey instanceof Map &&
            state.liveRemoteBadgesByKey.has(itemKey)
        ) {
            badges.push({
                label: "Changed",
                className: "is-remote",
                tooltip:
                    state.liveRemoteBadgesByKey.get(itemKey) ||
                    "Changed remotely",
            });
        }
    ...
    appendLiveBadges(container, state, itemKey) {
        ...
        for (const badge of this.getLiveBadgesForKey(state, itemKey)) {
            ...
            el.className =
                `tlr-live-badge text-details ${badge.className || ""}`.trim();
    ...
    syncStatusItem() {
        ...
        item.setTooltip?.(
            `Backreferences: ${snapshot.totalCount} total (${snapshot.pageCount} pages, ${snapshot.propertyCount} property, ${snapshot.linkedCount} linked)`,
        );
        item.show?.();
    }
    ...
    handleWorkspaceInvalidation(ev, reason) {
        ...
        this.refreshAllPanels({
            force: false,
            reason: reason || "workspace-invalidated",
        });
    }
    ...
    snapshotIncludesSourceRecord(state, recordGuid) {
        ...
        return (
            state?.liveCurrentSnapshot?.sourceRecordGuids?.has?.(guid) === true
        );
    }
    ...
    async refreshPanel(panelId, { reason }) {
        ...
        const maxResults = this.coercePositiveInt(
            cfg.custom?.maxResults,
            this._defaultMaxResults,
        );
        ...
        if (searchSettled.status === "fulfilled") {
            ...
            if (result?.error) {
                ...
                propertyCandidateRecords = Array.isArray(result?.records)
                    ? result.records
                    : null;
                linkedGroups = this.groupBacklinkLines(lines, recordGuid, {
                    showSelf,
                });
            }
        } else {
        ...
        let propertyGroups = [];
        try {
            propertyGroups = await this.getPropertyBacklinkGroups(
                record,
                recordGuid,
                {
                    showSelf,
        ...
        try {
            ...
            if (recordName) {
                ...
                const unlinkedSearchResult = await this.data.searchByQuery(
                    recordName,
                    maxResults,
                );

                if (!this._panelStates.has(panelId) || state.refreshSeq !== seq)
                    return;

                if (unlinkedSearchResult?.error) {
                    unlinkedError = unlinkedSearchResult.error;
                } else {
                    const allLines = Array.isArray(unlinkedSearchResult?.lines)
                        ? unlinkedSearchResult.lines
                        : [];
                    ...
                    for (const line of allLines) {
                        ...
                        if (!this.lineHasTextMentionOf(line, recordName))
                            continue;
                        candidateLines.push(line);
                    }
                    unlinkedGroups = this.groupBacklinkLines(
                        candidateLines,
                        recordGuid,
                        { showSelf },
                    );
        ...
        this.applyLiveSnapshot(
            state,
            this.buildResultsSnapshot(propertyGroups, linkedGroups),
        );
    ...
    handleLineItemUpdated(ev) {
        ...
        const segments =
            ev?.hasSegments?.() && typeof ev.getSegments === "function"
                ? ev.getSegments() || []
                : [];
        ...
        for (const state of this._panelStates.values()) {
            ...
            const hitsKnownSource = this.snapshotIncludesSourceRecord(
                state,
                editedRecordGuid,
            );
            ...
            if (editedRecordGuid && editedRecordGuid === state.recordGuid)
                continue;
    ...
    handleLineItemDeleted(ev) {
        ...
        for (const state of this._panelStates.values()) {
            ...
            this.scheduleRefreshForPanel(panel, {
                force: false,
                reason: "lineitem.deleted",
                debounceMs: this._refreshDebounceMs,
            });
        }
    }
    ...
    getLinkedContextState(state, lineGuid) {
        ...
        if (!(state.linkedContextByLine instanceof Map))
            state.linkedContextByLine = new Map();
    ...
    getVisibleAboveContextItems(ctx) {
        if (!ctx || ctx.loaded !== true || !Array.isArray(ctx.aboveItems))
            return [];
        const available = this.getAvailableAboveContextCount(ctx) || 0;
        const count = Math.max(
            0,
            Math.min(ctx.siblingAboveCount || 0, available),
        );
        if (count === 0) return [];
    ...
    getVisibleBelowContextItems(ctx) {
        if (!ctx || ctx.loaded !== true || !Array.isArray(ctx.belowItems))
            return [];
        const available = this.getAvailableBelowContextCount(ctx) || 0;
        const count = Math.max(
            0,
            Math.min(ctx.siblingBelowCount || 0, available),
        );
        if (count === 0) return [];
    ...
    findLinkedLineByGuid(state, lineGuid) {
        ...
        const linked = Array.isArray(state.lastResults?.linkedGroups)
            ? state.lastResults.linkedGroups
            : [];
        const unlinked = Array.isArray(state.lastResults?.unlinkedGroups)
            ? state.lastResults.unlinkedGroups
            : [];
    ...
    buildRecordDocumentOrder(record, items) {
        ...
        for (const item of list) {
            ...
            const parentGuid =
                typeof item?.parent_guid === "string" && item.parent_guid
                    ? item.parent_guid
                    : recordGuid;
    ...
    async ensureLinkedContextLoaded(state, line) {
        ...
        ctx.loadPromise = (async () => {
            ...
            const allItems =
                record && typeof record.getLineItems === "function"
                    ? (await record.getLineItems(false)) || []
                    : [];
            const orderedItems = this.buildRecordDocumentOrder(
                record,
                allItems,
            );
            const contextItems =
                orderedItems.length > 0 ? orderedItems : allItems;
            const matchedGuid = line?.guid || "";
            const matchedIndex = contextItems.findIndex(
                (item) => (item?.guid || "") === matchedGuid,
            );
            const aboveItems = [];
            const belowItems = [];
            ...
            ctx.siblingAboveCount = Math.max(
                0,
                Math.min(ctx.siblingAboveCount || 0, availableAbove || 0),
            );
            ctx.siblingBelowCount = Math.max(
                0,
                Math.min(ctx.siblingBelowCount || 0, availableBelow || 0),
            );
            return ctx;
        })()
            .catch(() => {
    ...
    async handleLinkedContextAction(state, action, lineGuid) {
        ...
        if (action === "toggle-context-more") {
            ...
            ctx.siblingAboveCount = this.adjustContextWindowCount(
                ctx.siblingAboveCount,
                this.getAvailableAboveContextCount(ctx),
            );
        } else if (action === "toggle-context-below") {
            ctx.siblingBelowCount = this.adjustContextWindowCount(
                ctx.siblingBelowCount,
                this.getAvailableBelowContextCount(ctx),
            );
        } else {
            return;
        }
    ...
    async getPropertyBacklinkGroups(
        _targetRecord,
        targetGuid,
        { showSelf, candidateRecords },
    ) {
        ...
        const groups = Array.from(byProp.entries()).map(
            ([propertyName, recordMap]) => ({
                propertyName,
    ...
    groupBacklinkLines(lines, targetGuid, { showSelf }) {
        ...
        for (const line of lines || []) {
            ...
            const prev = byRecord.get(srcGuid) || {
                record: srcRecord,
                lines: [],
            };
    ...
    sortPropertyGroupsForRender(groups, sortSpec, sortMetrics) {
        return (groups || []).map((g) => {
            const records = Array.isArray(g?.records)
                ? Array.from(g.records)
                : [];
            records.sort((a, b) =>
                this.compareRecordsForSort(a, b, sortSpec, sortMetrics),
            );
            return {
        ...
        for (const g of propertyGroups || []) {
            for (const record of g?.records || []) {
                ...
                setReferenceActivity(
                    guid,
                    this.getRecordUpdatedTimestamp(record),
                );
            }
        }
    ...
    compareRecordsForSort(a, b, sortSpec, sortMetrics) {
        const sortBy =
            this.normalizeSortBy(sortSpec?.sortBy) || this._defaultSortBy;
        const sortDir =
            this.normalizeSortDir(sortSpec?.sortDir) || this._defaultSortDir;
        ...
        if (sortBy === "page_title") {
            primary = this.compareText(
                this.getRecordNameForSort(a),
                this.getRecordNameForSort(b),
            );
        } else if (sortBy === "page_created_date") {
            primary = this.compareNumbers(
                this.getRecordCreatedTimestamp(a),
                this.getRecordCreatedTimestamp(b),
            );
            ...
            primary = this.compareNumbers(
                this.getRecordUpdatedTimestamp(a),
                this.getRecordUpdatedTimestamp(b),
            );
        }
        ...
        const nameTieBreak = this.compareText(
            this.getRecordNameForSort(a),
            this.getRecordNameForSort(b),
        );
    ...
    getReferenceActivityTimestamp(record, sortMetrics) {
        ...
        if (!guid) return 0;
        const fromLinked =
            sortMetrics?.referenceActivityByGuid?.get?.(guid) || 0;
    ...
    renderReferences(
        ...
        const pinnedPhrases = (state.searchPhrases || [])
            .map((p) => p.toLowerCase())
            .filter(Boolean);
        const typedPhrase = (state.searchTyped || "").trim().toLowerCase();
        const phrases = typedPhrase
            ? [...pinnedPhrases, typedPhrase]
            : [...pinnedPhrases];
        ...
        const totalPropRefCount = propsAll.reduce(
            (n, g) => n + (g?.records?.length || 0),
            0,
        );
        ...
        const isEmptyWithoutFilter =
            !hasAnyErrors &&
            totalPropRefCount === 0 &&
            totalLinkedRefCount === 0 &&
            totalUnlinkedRefCount === 0;
        const useCompactEmpty =
        ...
        const totalUniquePages = new Set([
            ...totalUniqueLinkedPages,
            ...totalUniqueUnlinkedPages,
        ]);
        ...
        if (hasFilter) {
            ...
            for (const g of propsAll) {
                ...
                if (recs.length > 0)
                    nextProps.push({ propertyName, records: recs });
            }
            props = nextProps;
        ...
        const filteredUniquePages = new Set([
            ...filteredUniqueLinkedPages,
            ...filteredUniqueUnlinkedPages,
        ]);
        ...
        unlinked = this.sortLinkedGroupsForRender(
            unlinked,
            sortSpec,
            sortMetrics,
        );
        ...
        if (hasFilter) {
            const shortDisplay =
                queryDisplay.length > 30
                    ? `${queryDisplay.slice(0, 30)}…`
                    : queryDisplay;
            parts.push(`Filter: "${shortDisplay}"`);
            if (totalUniquePages.size > 0)
                parts.push(
                    `${filteredUniquePages.size}/${totalUniquePages.size} pages`,
                );
            if (totalLinkedRefCount > 0)
                parts.push(
                    `${filteredLinkedRefCount}/${totalLinkedRefCount} linked`,
                );
            if (totalUnlinkedRefCount > 0)
                parts.push(
                    `${filteredUnlinkedRefCount}/${totalUnlinkedRefCount} unlinked`,
                );
        } else {
            if (totalUniquePages.size > 0)
                parts.push(
                    `${totalUniquePages.size} page${totalUniquePages.size === 1 ? "" : "s"}`,
                );
            if (totalLinkedRefCount > 0)
                parts.push(`${totalLinkedRefCount} linked`);
            if (totalUnlinkedRefCount > 0)
                parts.push(`${totalUnlinkedRefCount} unlinked`);
        }
        ...
        if (propertyError) {
            ...
            this.appendEmpty(
                propBody,
                hasFilter
                    ? "No matching property references."
                    : "No property references.",
            );
        } else {
            this.appendPropertyReferenceGroups(propBody, props, {
                query,
                state,
            });
        }
        ...
        const unlinkedBlock = this.buildSectionBlock({
            ...
            extraHeaderContent:
                filteredUnlinkedRefCount > 1
                    ? this.buildLinkAllButton(filteredUnlinkedRefCount)
                    : null,
        });
    ...
    buildSectionBlock({
        ...
        header.type = "button";
        header.className =
            "tlr-section-header button-normal button-normal-hover";
    ...
    appendUnlinkedReferenceGroups(container, groups, opts) {
        ...
        const query = Array.isArray(opts?.query)
            ? opts.query
        ...
        for (const g of groups || []) {
            ...
            header.type = "button";
            header.className =
                "tlr-group-header button-normal button-normal-hover";
            ...
            for (const line of g.lines || []) {
                ...
                const ctx = state
                    ? this.getLinkedContextState(state, line.guid)
                    : null;
                if (
                    state &&
                    ctx &&
                    this.hasRequestedLinkedContext(ctx) &&
                    ctx.loaded !== true &&
                    ctx.loading !== true
                ) {
                ...
                if (state && ctx) {
                    actionsRow.appendChild(
                        this.buildLinkedContextControls(line.guid, ctx),
                    );
                }
    ...
    renderRecordPreview(previewEl, allItems, recordGuid, collapsedNodes) {
        ...
        const renderNode = (item, depth) => {
            ...
            if (hasChildren) {
                ...
                arrow.className =
                    "tlr-preview-arrow" + (isCollapsed ? " is-collapsed" : "");
        ...
        for (const seg of line?.segments || []) {
            if (
                ...
                if (
                    ...
                    seg.text.toLowerCase().includes(nameLower)
                )
                    return true;
            }
        }
    ...
    async linkUnlinkedReference(state, lineGuid) {
        ...
        const newSegments = this.buildReplacedSegments(
            segments,
            recordName,
            targetRecordGuid,
        );
    ...
    async linkAllUnlinkedReferences(state) {
        ...
        if (hasFilter) {
            ...
            const allMatch = (text) =>
                phrases.every((p) => wordStartRe(p).test(text));
            groups = [];
    ...
    saveRecordGroupCollapsedSetting() {
        try {
            localStorage.setItem(
                this._storageKeyRecordGroupCollapsed,
                JSON.stringify(this._recordGroupCollapsed || {}),
            );
        } catch (e) {
            // ignore
        }
    ...
    appendCompactEmptyState(container) {
        ...
        expandBtn.className =
            "tlr-empty-compact-btn button-none button-small button-minimal-hover";
    ...
    appendPropertyReferenceGroups(container, groups, opts) {
        ...
        for (const g of groups || []) {
            ...
            header.type = "button";
            header.className =
                "tlr-prop-header button-normal button-normal-hover";
            ...
            for (const r of g?.records || []) {
                ...
                if (
                    state?.recordExpandedState?.get?.(guid)?.expanded === true
                ) {
                ...
                this.appendLiveBadges(
                    btn,
                    state,
                    this.getPropertySnapshotKey(propName, guid),
                );
    ...
    appendLinkedReferenceGroups(container, groups, opts) {
        ...
        const totalLineCount =
            typeof opts?.totalLineCount === "number"
                ? opts.totalLineCount
                : null;
        const emptyMessage =
            (opts?.emptyMessage || "").trim() || "No linked references.";

        const pageCount = groups.length;
        const refCount = groups.reduce(
            (n, g) => n + (g?.lines?.length || 0),
            0,
        );
        ...
        for (const g of groups) {
            ...
            for (const line of g.lines || []) {
                ...
                this.appendLiveBadges(
                    lineEl,
                    state,
                    this.getLinkedSnapshotKey(line.guid),
                );
    ...
    buildLinkedContextControls(lineGuid, ctx) {
        ...
        if (ctx?.showMoreContext === true) {
            group.appendChild(
                this.buildLinkedContextButton(
                    "toggle-context-above",
                    lineGuid,
                    {
                        icon: "up",
        ...
        group.appendChild(
            this.buildLinkedContextButton("toggle-context-more", lineGuid, {
                icon: "toggle",
                label:
                    ctx?.showMoreContext === true
                        ? "Hide context"
    ...
    buildLinkedContextButton(action, lineGuid, opts) {
        ...
        btn.type = "button";
        btn.className =
            "tlr-context-btn button-none button-small button-minimal-hover";
    ...
    appendLinkedContextRows(container, recordGuid, ctx, query, position) {
        ...
        for (const item of items) {
            ...
            row.style.setProperty(
                "--tlr-context-indent",
                `${Math.max(0, item.indent || 0) * 12}px`,
            );
    ...
    segmentsToPlainText(segments) {
        ...
        for (const seg of segments) {
            ...
            if (seg.type === "ref") {
                ...
                const title =
                    seg.text?.title ||
                    (guid ? this.resolveRecordName(guid) : "") ||
                    "";
                out += title;
                continue;
            }
        ...
        for (const [start, end] of merged) {
            if (start > pos)
                container.appendChild(
                    document.createTextNode(s.slice(pos, start)),
                );
    ...
    appendSegments(container, segments, query) {
        ...
        for (const seg of segments) {
            ...
            if (seg.type === "ref") {
                ...
                const title =
                    seg.text?.title || this.resolveRecordName(guid) || "[link]";
                el.textContent = "";
